...
pub struct CRR1CSProof<E: Pairing<ScalarField=F>, PC: KZH<E>, F: PrimeField + Absorb> {
    /// Sumcheck proof for the polynomial g(x) = \sum eq(tau,x) * (~Az_masked~(x) * ~Bz_masked~(x) - ~Cz_masked~(x))
    pub sc_proof_phase1: SumcheckInstanceProof<F>,
    /// Evaluation claims for ~Az_masked~(rx), ~Bz_masked~(rx), and ~Cz_masked~(rx).
    pub claims_phase2: (F, F, F),
    /// Sumcheck proof for the polynomial F(x) = ~Z_masked(x)~ * ~ABC~(x), where ABC(x) = \sum_t ~M~(t,x) eq(r,t)
    /// for M a random linear combination of A, B, and C.
    pub sc_proof_phase2: SumcheckInstanceProof<F>,
    /// The claimed evaluation ~Z_masked~(ry)
    pub eval_vars_at_ry: F,
    /// A polynomial evaluation proof of the claimed evaluation ~Z_masked~(ry) with respect to the commitment comm_W.
    pub proof_eval_vars_at_ry: PC::Opening,
    /// The poly commitment to the masking for witness
    pub mask_commitment: PC::Commitment,
    /// The claimed evaluation ~Z_{mask}~(ry)
    pub eval_mask_at_ry: F,
    /// A polynomial evaluation proof of the claimed evaluation ~Z_{mask}~(ry) with respect to the commitment comm_W.
    pub proof_eval_mask_at_ry: PC::Opening,
...
impl<E: Pairing<ScalarField=F>, PC: KZH<E>, F: PrimeField + Absorb> CRR1CSProof<E, PC, F> {
    ...
    fn prove_phase_one(
        num_rounds: usize,
        evals_tau: &mut MultilinearPolynomial<F>,
        evals_Az_masked: &mut MultilinearPolynomial<F>,
        evals_Bz_masked: &mut MultilinearPolynomial<F>,
        evals_Cz_masked: &mut MultilinearPolynomial<F>,
        transcript: &mut Transcript<F>,
    ) -> (
        SumcheckInstanceProof<F>,
        Vec<F>,
    ...
    fn prove_phase_two(
        num_rounds: usize,
        claim: &F,
        evals_z_masked: &mut MultilinearPolynomial<F>,
        evals_ABC: &mut MultilinearPolynomial<F>,
        transcript: &mut Transcript<F>,
    ) -> (
        SumcheckInstanceProof<F>,
        ...
        let comb_func = |poly_A_comp: &F,
                         ...
                         -> F { poly_A_comp.clone() * poly_B_comp.clone() };
        let (sc_proof_phase_two, r, claims) = SumcheckInstanceProof::prove_quad::<_, E>(
            claim, num_rounds, evals_z_masked, evals_ABC, comb_func, transcript,
        );
        ...
        <<E as Pairing>::G1Affine as ark_ec::AffineRepr>::BaseField: PrimeField
    {
        ...
        // Random mask for vars
        let mut rng = thread_rng();
        let witness_mask: Vec<F> = (0..vars.len()).map(|_| F::rand(&mut rng)).collect();
        let witness_poly_mask = MultilinearPolynomial {
            num_variables: vars.len().log_2(),
            evaluation_over_boolean_hypercube: witness_mask.clone(),
            len: vars.len(),
        };

        let (poly_mask_com, poly_mask_aux) = PC::commit(srs, &witness_poly_mask, &mut thread_rng());
        ...
        let z_mask = {
            // Padding zeros same as in z
            let padding_len = vars.len() - input.len() - 1;
            let input_mask = vec![F::zero(); input.len()];
            let zeros_mask = vec![F::zero(); padding_len];

            // Build z_mask
            let mut z_mask = witness_mask;
            z_mask.push(F::zero()); // relaxed constant term mask (0 instead of 1)
            z_mask.extend(input_mask);
            z_mask.extend(zeros_mask);
            z_mask
        };

        transcript.append_point::<E>(b"commitment of z_mask", &poly_mask_com.to_affine());
        let rho = transcript.challenge_scalar(b"rho");
        let z_masked: Vec<F> = z_mask
            .iter()
            .zip(z.iter())
            .map(|(zmi, zi)| *zmi + rho * zi)
            .collect();
        ...
        // compute the initial evaluation table for R(\tau, x)
        let (mut poly_Az_masked, mut poly_Bz_masked, mut poly_Cz_masked) =
            inst.multiply_vec(inst.get_num_cons(), z_masked.len(), &z_masked);

        let mut poly_tau = MultilinearPolynomial::new(EqPolynomial::new(tau).evals());

        let (sc_proof_phase1, rx, claims_phase1) = CRR1CSProof::<E, PC, F>::prove_phase_one(
            num_rounds_x,
            &mut poly_tau,
            &mut poly_Az_masked,
            &mut poly_Bz_masked,
        ...
        assert_eq!(poly_tau.len(), 1);
        assert_eq!(poly_Az_masked.len(), 1);
        assert_eq!(poly_Bz_masked.len(), 1);
        assert_eq!(poly_Cz_masked.len(), 1);
        timer_sc_proof_phase1.stop();

        let (_, Az_masked_claim, Bz_masked_claim, Cz_masked_claim) = (
            &poly_tau[0],
            &poly_Az_masked[0],
            &poly_Bz_masked[0],
            &poly_Cz_masked[0],
        ...
        Transcript::append_scalar(transcript, b"Az_claim", Az_masked_claim);
        Transcript::append_scalar(transcript, b"Bz_claim", Bz_masked_claim);
        Transcript::append_scalar(transcript, b"Cz_claim", Cz_masked_claim);
        ...
        let r_B = Transcript::challenge_scalar(transcript, b"challenege_Bz");
        let r_C = Transcript::challenge_scalar(transcript, b"challenege_Cz");
        let claim_phase2 = r_A * Az_masked_claim + r_B * Bz_masked_claim + r_C * Cz_masked_claim;

        let evals_ABC = {
            // compute the initial evaluation table for R(\tau, x)
            let evals_rx = EqPolynomial::new(rx.clone()).evals();
            let (evals_A, evals_B, evals_C) = inst.compute_eval_table_sparse(inst.get_num_cons(), z_masked.len(), &evals_rx);
        ...
        assert!(z_masked.len().is_power_of_two(), "z must be a power of two");
        assert!(evals_ABC.len().is_power_of_two(), "eval_ABC must be a power of two");
        assert_eq!(z_masked.len(), evals_ABC.len(), "vector z and eval_ABC should have equal length");

        // another instance of the sumcheck protocol
        let (sc_proof_phase2, ry, _claims_phase2) = CRR1CSProof::<E, PC, F>::prove_phase_two(
            num_rounds_y,
            &claim_phase2,
            &mut MultilinearPolynomial::new(z_masked),
            &mut MultilinearPolynomial::new(evals_ABC),
            transcript,
        );
        timer_sc_proof_phase2.stop();
        ...
        let timer_polyeval = Timer::new("poly eval");
        let eval_vars_at_ry = poly_vars.evaluate(&ry[1..]);
        let eval_mask_at_ry = witness_poly_mask.evaluate(&ry[1..]);
        timer_polyeval.stop();
        ...
        (
            CRR1CSProof {
                sc_proof_phase1,
                claims_phase2: (*Az_masked_claim, *Bz_masked_claim, *Cz_masked_claim),
                sc_proof_phase2,
                eval_vars_at_ry,
                proof_eval_vars_at_ry,
                mask_commitment: poly_mask_com,
                eval_mask_at_ry,
                proof_eval_mask_at_ry,
            },
            rx,
            ry,
        )
        ...
        <<E as Pairing>::G1Affine as ark_ec::AffineRepr>::BaseField: PrimeField
    {
        ...
        transcript.append_point::<E>(b"commitment of z_mask", &self.mask_commitment.to_affine());
        let rho = transcript.challenge_scalar(b"rho");
        ...
        // perform the intermediate sum-check test with claimed Az, Bz, Cz, and E
        let (Az_masked_claim, Bz_masked_claim, Cz_masked_claim) = self.claims_phase2;
        ...
        let expected_claim_post_phase1 = (Az_masked_claim * Bz_masked_claim - Cz_masked_claim) * taus_bound_rx;
        //assert_eq!(expected_claim_post_phase1, claim_post_phase1);
        ...
        // compute Z(r_y): eval_Z_at_ry = (F::one() - ry[0]) * self.eval_vars_at_ry + ry[0] * poly_input_eval
        let eval_Z_masked_at_ry = (F::one() - ry[0]) * (self.eval_vars_at_ry * rho + self.eval_mask_at_ry) + ry[0] * poly_input_eval;

        // perform the final check in the second sum-check protocol
        let (eval_A_r, eval_B_r, eval_C_r) = evals;
        let expected_claim_post_phase2 = eval_Z_masked_at_ry * (r_A * eval_A_r + r_B * eval_B_r + r_C * eval_C_r);
...
mod tests {
    ...
    fn check_crr1cs_proof_helper<E: Pairing, PC: KZH<E>>()
    where
        <E as Pairing>::ScalarField: Absorb, <<E as Pairing>::G1Affine as ark_ec::AffineRepr>::BaseField: PrimeField
    {
        let num_vars = 1024;
        let num_cons = num_vars;
        let num_inputs = 10;
